...
그런데 $F(2k+1) = F(k)^2 + F(k+1)^2$ 이 공식, 왜 저렇게 생겼는지 바로 와닿지 않을 수 있다.  
아래 그림으로 직관을 잡고 가자.
...
> $F(2k+1) = F(k)^2 + F(k+1)^2$ 공식을 설명할 수 있다.  
...
- **경우 ①** — 딱 k에서 나눔: 왼쪽 F(k)가지 × 오른쪽 F(k)가지 = $F(k)^2$
- **경우 ②** — k+1에서 나눔: 왼쪽 F(k+1)가지 × 오른쪽 F(k+1)가지 = $F(k+1)^2$

이 두 경우를 더하면 $F(2k+1) = F(k)^2 + F(k+1)^2$ 이 나온다.  
제곱이 등장하는 이유는 **양쪽이 대칭이라 같은 수를 곱하기 때문**이다.
