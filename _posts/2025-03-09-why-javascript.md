...
> "야, 파이썬 그렇게 쉽고 좋다면서 왜 웹 만들 때는 안 써? 왜 자바스크립트 쓰는 거야?"

순간 말문이 막혔다. 당연하게 써왔으니까. 근데 막상 '왜?'를 설명하려니 생각보다 할 말이 많았다.
...
웹 페이지가 살아있으려면 자바스크립트가 필요해졌다.

굳이 친구 질브라우저가 파이썬을 실행하는 기능을 넣지 않았으니, 아무리 파이썬이 좋아도 브라우저 위에서 직접 돌릴 수 없었다.
즉, 웹 페이지가 사용자에게 전달된 이후 브라우저 안에서 직접 실행되는 언어는 사실상 **자바스크립트 하나뿐**이다.
